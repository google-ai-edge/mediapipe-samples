...
class MainViewModel : ViewModel() {
    ...
    fun setMinFaceLandmarkConfidence(confidence: Float) {
        val currentLandmarkConfidence =
            helperState.value?.minFaceLandmarkThreshold ?: 0f
        helperState.value = helperState.value?.copy(
            minFaceLandmarkThreshold = ((currentLandmarkConfidence.toBigDecimal() + confidence.toBigDecimal()).toFloat()).coerceIn(
                0f,
                1f
            )
        )
    ...
    fun setMinPoseLandmarkConfidence(confidence: Float) {
        val currentLandmarkConfidence =
            helperState.value?.minPoseLandmarkThreshold ?: 0f
        helperState.value =
            helperState.value?.copy(
                minPoseLandmarkThreshold = ((currentLandmarkConfidence.toBigDecimal() + confidence.toBigDecimal()).toFloat()).coerceIn(
                    0f, 1f
                )
            )
    }
...
data class HelperState(
    ...
    val minFaceLandmarkThreshold: Float = HolisticLandmarkerHelper.DEFAULT_MIN_FACE_LANDMARKS_CONFIDENCE,
    val minHandLandmarkThreshold: Float = HolisticLandmarkerHelper.DEFAULT_MIN_HAND_LANDMARKS_CONFIDENCE,
    val minPoseLandmarkThreshold: Float = HolisticLandmarkerHelper.DEFAULT_MIN_POSE_LANDMARKS_CONFIDENCE,
