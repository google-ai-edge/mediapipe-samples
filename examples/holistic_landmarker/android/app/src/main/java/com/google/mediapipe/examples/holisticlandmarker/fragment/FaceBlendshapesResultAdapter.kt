...
class FaceBlendshapesResultAdapter :
    ...
    fun updateResults(holisticLandmarkerResult: HolisticLandmarkerResult? = null) {
        categories = MutableList(52) { null }
        if (holisticLandmarkerResult != null && holisticLandmarkerResult.faceBlendshapes().isPresent) {
            val faceBlendshapes = holisticLandmarkerResult.faceBlendshapes().get()
            val sortedCategories = faceBlendshapes.sortedByDescending { it.score() }
