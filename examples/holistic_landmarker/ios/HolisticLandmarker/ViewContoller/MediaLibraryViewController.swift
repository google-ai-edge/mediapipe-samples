 ...
 * presenting them with the landmarks of holistic to the user.
 */
class MediaLibraryViewController: UIViewController {
  ...
  private struct Constants {
    ...
    "Click + to add an image or a video to begin running the holistic landmark."
  ...
  override func viewWillLayoutSubviews() {
    ...
    redrawOverlaysForCurrentDeviceOrientation()
  }
  ...
  private func redrawOverlaysForCurrentDeviceOrientation() {
    overlayView.redrawHolisticOverlays(forNewDeviceOrientation: UIDevice.current.orientation)
  }

  func clearAndInitializeHolisticLandmarkerService(runningMode: RunningMode) {
    holisticLandmarkerService = nil
    switch runningMode {
    case .image:
      holisticLandmarkerService = HolisticLandmarkerService.stillImageLandmarkerService(
...
extension MediaLibraryViewController: UIImagePickerControllerDelegate, UINavigationControllerDelegate {
  ...
  func imagePickerController(
    ...
    case UTType.image.identifier:
      ...
      let image = rawImage.normalizedImage()
      ...
      DispatchQueue.global(qos: .userInteractive).async { [weak self] in
        guard let weakSelf = self,
              ...
              let holisticResult = resultBundle.holisticLandmarkerResults.first,
              let holisticResult = holisticResult else {
          DispatchQueue.main.async {
        ...
        DispatchQueue.main.async {
          ...
          let holisticOverlays = OverlayView.holisticOverlays(
          ...
          weakSelf.overlayView.draw(
            holisticOverlays: holisticOverlays,
  ...
  private func playVideo(mediaURL: URL, videoDuration: Double, resultBundle: ResultBundle?) {
    ...
    playerTimeObserverToken = playerViewController?.player?.addPeriodicTimeObserver(
      forInterval: CMTime(
        value: Int64(Constants.inferenceTimeIntervalInMilliseconds),
        timescale: Int32(Constants.milliSeconds)),
      queue: DispatchQueue(
        label: "com.google.mediapipe.MediaLibraryViewController.timeObserverQueue",
        qos: .userInteractive),
      using: { [weak self] (time: CMTime) in
        DispatchQueue.main.async {
          ...
          guard
            ...
            let holisticResult = resultBundle.holisticLandmarkerResults[index] else {
            return
          }
...
// MARK: HolisticLandmarkerServiceVideoDelegate Methods
