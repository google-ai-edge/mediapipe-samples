...
class RootViewController: UIViewController {
  ...
  private struct Constants {
    static let inferenceBottomHeight = 300.0
  ...
  override func viewDidLoad() {
    super.viewDidLoad()
    // Create Gesture Recognizer helper
...
extension RootViewController: InferenceResultDeliveryDelegate {
  func didPerformInference(result: ResultBundle?, index: Int) {
    ...
    if let result = result,
       index < result.gestureRecognizerResults.count,
       let gestureRecognizerResult = result.gestureRecognizerResults[index],
       let category = gestureRecognizerResult.gestures.first?.first {
      inferenceViewController?.update(inferenceTimeString: inferenceTimeString,
                                      recognizationResultString: category.categoryName,
                                      score: category.score)
    } else {
      inferenceViewController?.update(inferenceTimeString: inferenceTimeString,
                                      recognizationResultString: nil,
                                      score: nil)
