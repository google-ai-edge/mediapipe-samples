...
class ObjectDetectorHelper(
  ...
  fun setupObjectDetector() {
      ...
      RunningMode.VIDEO -> {
        if (objectDetectorListener != null) {
          throw IllegalStateException(
            "objectDetectorListener cannot be set when runningMode is IMAGE or VIDEO."
          )
        }
      }
    ...
    try {
      ...
      when (runningMode) {
        ...
        RunningMode.LIVE_STREAM ->
          optionsBuilder.setRunningMode(runningMode).setResultListener(this::returnLivestreamResult)
  ...
  fun detectVideoFile(videoUri: Uri, inferenceIntervalMs: Long): ResultBundle? {
    ...
    if(runningMode != RunningMode.VIDEO) {
      throw IllegalArgumentException("Attempting to call detectVideoFile" +
        ...
        ?.let { frame ->
          ...
          objectDetector?.detectForVideo(mpImage, timestampMs)?.let { detectionResult ->
            ...
            ?: {
              ...
              objectDetectorListener?.onError("ResultBundle could not be returned" +
                      " in detectVideoFile")
            }
        }
        ?: run {
          didErrorOccurred = true
          objectDetectorListener?.onError("Frame at specified time could not be" +
  ...
  fun detectLivestreamFrame(imageProxy: ImageProxy) {
    ...
    if(runningMode != RunningMode.LIVE_STREAM) {
      throw IllegalArgumentException("Attempting to call detectLivestreamFrame" +
  ...
  private fun returnLivestreamResult(result: ObjectDetectionResult, input: MPImage) {
    ...
    val inferenceTime = finishTimeMs - (result.timestampMs() / 1000)
