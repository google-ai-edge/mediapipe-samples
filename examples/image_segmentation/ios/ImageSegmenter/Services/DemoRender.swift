...
class DemoRender {
  ...
  func render(pixelBuffer: CVPixelBuffer, segmentDatas: [Float32]?) -> CVPixelBuffer? {
    ...
    let buffer = metalDevice.makeBuffer(bytes: segmentDatas, length: segmentDatas.count * MemoryLayout<Float32>.size)!
